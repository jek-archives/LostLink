...
* **Real-time Item Feed:** Report and view lost and found items instantly (synced with Firebase Firestore).
...
* **Demo Sandbox Mode:** Built-in offline testing sandbox utilizing `localStorage` to completely simulate backend flows without needing Firebase authentication redirect configuration.
...
If you are developing locally and haven't configured the Google OAuth authorized redirect domains inside your Firebase Console, you can bypass auth using **Try Demo Sandbox Mode**:
* **Instant Login:** Click the sandbox button on the landing page to sign in immediately as `Demo Student`.
* **Isolated Environment:** Actions like reporting new items and resolving claims will write to `localStorage` instead of Firestore, preventing permission or network errors.
